...
def safe_json_loads(json_string: str) -> dict: # FIXME:
    """Безопасно извлекает JSON из ответа LLM, удаляя markdown-обертки и лишний текст."""
    try:
        # Удаляем маркеры markdown, если модель их добавила
        cleaned = re.sub(r'^```json\s*', '', json_string.strip(), flags=re.MULTILINE)
        cleaned = re.sub(r'\s*```$', '', cleaned, flags=re.MULTILINE)
        # Берем только первую валидную JSON-подстроку (от первой { до последней })
        match = re.search(r'\{.*\}', cleaned, re.DOTALL)
        if match:
            return json.loads(match.group())
        return json.loads(cleaned)
    except json.JSONDecodeError as e:
        logger.error(f"Не удалось распарсить JSON от LLM. Сырой ответ: {json_string}")
        raise e
...
# web_search_tool = TavilySearchResults(k=3) # FIXME:

# Инструмент веб-поиска через DuckDuckGo (бесплатно)
from langchain_community.tools import DuckDuckGoSearchRun

def web_search_fallback(query: str):
    """Бесплатный веб-поиск без API ключа"""
    try:
        search = DuckDuckGoSearchRun()
        result = search.run(query)
        return [{"content": result, "title": "Web Search"}]
    except Exception as e:
        logger.error(f"Web search failed: {e}")
        return []

web_search_tool = web_search_fallback

# Инициализация модели LLM для генерации ответов
logger.debug('LLM')
from langchain_ollama import ChatOllama
local_llm = "qwen2.5-coder:1.5b"
...
def get_index_db():
    ...
    if os.path.exists(file_path):
        ...
        text_splitter = RecursiveCharacterTextSplitter(chunk_size=1024, chunk_overlap=0) # FIXME:
...
def grade_documents(state):
    ...
    for d in documents:
        ...
        grade = safe_json_loads(result.content)["binary_score"] # FIXME:
        # Document relevant
        if grade.lower() == "yes":
...
with open("graph_image.png", "wb") as png: # FIXME:
    png.write(graph_image)
print("✅ Схема графа успешно сохранена в файл 'graph_image.png'")
...
if __name__ == "__main__":
    ...
    inputs = {"question": "Что такое сортировка? и какую роль играет в алгоритмах ?", "max_retries": 3}
    print("🚀 Запуск графа...")
    for event in graph.stream(inputs, stream_mode="values"):
        logger.debug(event)
        # Выводим только финальный ответ или ключевые шаги, чтобы не засорять консоль
        if "generation" in event:
            print("\n" + "="*50)
            print("ОТВЕТ МОДЕЛИ:", event["generation"].content)
            print("="*50 + "\n")
